...
int main(MAPI_UNUSED int argc, char *argv[])
{
	...
	DOT;
	if(mapi_apply_function(fd, "ANONYMIZE", "TCP,SEQUENCE_NUMBER,HASHED,SHA,PAD_WITH_ZERO") < 0) {
		printf("Could not apply ANONYMIZE TCP,SEQUENCE_NUMBER,HASHED,SHA,PAD_WITH_ZERO \n");
		mapi_read_error( &err_no, error);
		fprintf(stderr,"Errorcode :%d description: %s \n" ,err_no, error);
		return -1;
	}
