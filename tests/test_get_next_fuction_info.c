...
DOT;	
	if((fid2=mapi_apply_function(fd, "BYTE_COUNTER"))<0){
...
DOT;	
	if((res = mapi_get_next_function_info(fd, fid, &info))<0){
	  	fprintf(stderr, "mapi_get_next_function_info failed for flow %d\n", fd);
		mapi_read_error( &err_no, error);
		fprintf(stderr,"Errorcode :%d description: %s \n" ,err_no, error);
		return -1;
	}
...
DOT;	
	 ...
	 */
	if(res >= 0){
		if(info.name!=NULL && strcmp("BYTE_COUNTER" , info.name)!=0){
			fprintf(stderr, "WARNING: mapi_get_next_function_info failed\n");
		}
		else{
			printf("\nFunction fid=%d info:\n", fid);
			printf("\n\t name: %s", info.name);
...
DOT;
	...
	printf("\nmapi get_next_function_info function OK\n");
	/*
	 * Error checking
	 */
...
DOT;	
	if( (res = mapi_get_next_function_info(fd, 3123, &info)) !=0 ){
		mapi_read_error( &err_no, error);
...
DOT;
	if( (res = mapi_get_next_function_info(fd, fid, NULL)) !=0 ){
		mapi_read_error( &err_no, error);
...
DOT;	if((res = mapi_get_next_function_info(fd, fid, &info))<0){
	  	fprintf(stderr, "mapi_get_next_function_info failed for flow %d\n", fd);
		mapi_read_error( &err_no, error);
		fprintf(stderr,"Errorcode :%d description: %s \n" ,err_no, error);
		return -1;
...
DOT;
	...
	if(!flag)
		printf("\nMAPI get next function info Error Checking OK\n");
	else 
		printf("\nMAPI get next function info Error Checking :FAILED:\n");
return 0;
}
